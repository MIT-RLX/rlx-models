...
//! F5-TTS voice-clone synthesis → Whisper ASR round-trip. Needs the model
//! weights (RLX_F5TTS_DIR or weights/tts/f5tts) + Whisper; skips otherwise.
...
#[test]
fn f5tts_clone_roundtrip_via_whisper() {
    let (Some(dir), Some(wd)) = (model_dir(), whisper_dir()) else {
        eprintln!("skip: set RLX_F5TTS_DIR + RLX_WHISPER_DIR");
        return;
    };
    let tts = F5Tts::load_on(&dir, Device::Cpu).expect("load f5tts");
    let audio = tts
        .synthesize(TEXT, &read_prompt(), REF_TEXT, &InferOpts::default())
        .expect("synthesize");
    assert!(rlx_f5tts::peak_amplitude(&audio) > 0.05, "audio not audible");
