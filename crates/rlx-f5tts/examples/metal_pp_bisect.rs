//! Bisect F5_Preprocess CPU↔METAL + micro-op probes (GRN reduce, depthwise conv).
//!
//! ```bash
//! RLX_F5TTS_DIR=weights/tts/f5tts cargo run -p rlx-f5tts --release \
//!   --example metal_pp_bisect --features metal
...
fn main() -> anyhow::Result<()> {
    ...
    let mut metal_g =
        tiny.compile_named_with_options("F5_Preprocess", Device::Metal, d, &named, copts)?;
    ...
    let metal_out = metal_g.run_typed(&inputs);
    println!(
        "preprocess outputs: cpu={} metal={}",
        cpu_out.len(),
        metal_out.len()
    );
    for (i, ((cb, cdt), (gb, gdt))) in cpu_out.iter().zip(metal_out.iter()).enumerate() {
        let c = to_f32(cb, *cdt);
        let g = to_f32(gb, *gdt);
    ...
    if cpu_out.len() > 3 && metal_out.len() > 3 {
        let c = to_f32(&cpu_out[3].0, cpu_out[3].1);
        let g = to_f32(&metal_out[3].0, metal_out[3].1);
...
fn bisect_grn() -> anyhow::Result<()> {
    ...
    // ReduceL2 over axis 1: sqrt(sum(x^2, axis=1, keepdims)) → [1,1,C]
    {
        ...
        let cpu = Session::new(Device::Cpu)
            .compile(g.clone())
            .run_typed(&inputs);
        let metal = Session::new(Device::Metal).compile(g).run_typed(&inputs);
        let (cos, mae, mx) = stats(
            &to_f32(&cpu[0].0, cpu[0].1),
            &to_f32(&metal[0].0, metal[0].1),
        );
    ...
    // Depthwise conv: [1,C,1,L] × [C,1,1,7] groups=C
    {
        ...
        let (cos, mae, mx) = stats(
            ...
            &to_f32(&metal.run_typed(&inputs)[0].0, DType::F32),
        );
