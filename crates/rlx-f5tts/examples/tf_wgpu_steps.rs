//! Multi-step CPU vs wgpu DiT cosine drift (CPU preprocess reused).
use half::f16;
...
fn main() -> anyhow::Result<()> {
    ...
    // Widen conditioning to F32 for wgpu (f32 arena).
    ...
    for step in 0..steps {
        ...
        println!(
            "step {step}: cos={:.6} peak_cpu={:.4} peak_wgpu={:.4}",
            cos(&vc, &vm),
            peak(&vc),
            peak(&vm)
        );
        ...
        noise_cpu_dt = oc[0].1;
        noise_wgpu = as_f32_bytes(&om[0].0, om[0].1);
    }
