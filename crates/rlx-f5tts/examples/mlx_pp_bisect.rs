...
fn main() -> anyhow::Result<()> {
    ...
    let mut cuda_g =
        tiny.compile_named_with_options("F5_Preprocess", Device::Mlx, d, &named, copts)?;
...
fn bisect_grn() -> anyhow::Result<()> {
    ...
    // ReduceL2 over axis 1: sqrt(sum(x^2, axis=1, keepdims)) → [1,1,C]
    {
        ...
        let cpu = Session::new(Device::Cpu)
            .compile(g.clone())
            .run_typed(&inputs);
        let cuda = Session::new(Device::Mlx).compile(g).run_typed(&inputs);
    ...
    // Depthwise conv: [1,C,1,L] × [C,1,1,7] groups=C
    {
        ...
        cpu.set_param("w", &w);
        let mut cuda = Session::new(Device::Mlx).compile(g);
        cuda.set_param("w", &w);
