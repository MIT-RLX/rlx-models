//! Multi-step CPU vs MLX DiT cosine drift (CPU preprocess reused).
use half::f16;
...
fn main() -> anyhow::Result<()> {
    ...
    // Widen conditioning to F32 for MLX (f32 arena).
    ...
    let mut tf_mlx = tiny.compile_named("F5_Transformer", Device::Mlx, d, tf_named)?;

    // Path A: chained — each backend feeds its own previous output
    let mut noise_cpu = pre[0].0.clone();
    let mut noise_cpu_dt = pre[0].1;
    let mut noise_mlx = cond_f32[0].1.clone();
    ...
    for step in 0..steps {
        ...
        let feed_cpu: Vec<(&str, &[u8], DType)> = (0..6)
            ...
            .collect();
        let feed_mlx: Vec<(&str, &[u8], DType)> = (0..6)
            .map(|i| {
                let b = if i == 0 {
                    noise_mlx.as_slice()
                } else {
        ...
        let om = tf_mlx.run_typed(&feed_mlx);
        let vc = to_f32(&oc[0].0, oc[0].1);
        let vm = to_f32(&om[0].0, om[0].1);
        println!(
            "step {step}: cos={:.6} peak_cpu={:.4} peak_mlx={:.4}",
            cos(&vc, &vm),
            peak(&vc),
            peak(&vm)
        );
        ...
        noise_cpu_dt = oc[0].1;
        noise_mlx = as_f32_bytes(&om[0].0, om[0].1);
    }

    // Path B: teacher-forced — both backends always feed CPU previous output
