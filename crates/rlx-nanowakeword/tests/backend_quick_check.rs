use rlx_nanowakeword::{
    NanoWakeWordEngine, NanoWakeWordWeights, WakeConfig, available_devices, bind_streaming_device,
    score_wav,
};
...
fn backends_accept_stub() {
    for device in available_devices() {
        ...
        let mut eng = NanoWakeWordEngine::new(
            NanoWakeWordWeights::stub(true, "wake"),
            WakeConfig::default(),
        )
        .with_device_label(label);
        assert!(
            !score_wav(&mut eng, &vec![0.0f32; 1280 * 3])
                .unwrap()
                .is_empty(),
            "device={label}"
        );
