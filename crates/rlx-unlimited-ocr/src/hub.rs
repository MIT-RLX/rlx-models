    ...
    let config = repo.get("config.json").with_context(|| {
        format!(
            "locate {repo_id} in Hugging Face cache under {}\n\
             Download once: `huggingface-cli download {repo_id}` or `just fetch-unlimited-ocr`",
...
pub fn hf_snapshot_dir(_repo_id: &str) -> Result<PathBuf> {
    bail!(
        "Hugging Face cache support disabled; rebuild with `--features hf-cache` on rlx-unlimited-ocr"
    )
}
...
pub fn default_model_dir() -> Result<PathBuf> {
    ...
    if let Ok(p) = hf_snapshot_dir(UnlimitedOcrConfig::HF_MODEL_ID) {
        return Ok(p);
    }

    let local = PathBuf::from(".cache/unlimited-ocr/Unlimited-OCR");
    if local.join("config.json").is_file() {
        return Ok(local);
    }

    bail!(
        "Unlimited-OCR weights not found.\n\
         • Hugging Face cache: `huggingface-cli download {}` (or `just fetch-unlimited-ocr`)\n\
         • Or: `export RLX_UNLIMITED_OCR_DIR=/path/to/Unlimited-OCR`\n\
         Cache root: {}",
