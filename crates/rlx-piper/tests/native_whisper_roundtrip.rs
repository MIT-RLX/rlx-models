...
//! Native (ort-free) piper synthesis → Whisper ASR round-trip. Proves the
//! graph-split native runner produces intelligible speech on an RLX backend.
//! Set RLX_PIPER_DIR (or weights/tts/piper) + RLX_WHISPER_DIR. Needs the split
//! bundle (`scripts/split_piper.py`).
...
fn piper_dir() -> Option<PathBuf> {
    if let Ok(d) = std::env::var("RLX_PIPER_DIR") {
        ...
        if p.join("rlx-split/enc_p.onnx").is_file() {
            return Some(p);
        }
    }
    let p = PathBuf::from("weights/tts/piper");
    p.join("rlx-split/enc_p.onnx").is_file().then_some(p)
}
fn whisper_dir() -> Option<PathBuf> {
    ...
    ["whisper-base.en", "whisper-small.en", "whisper-tiny"]
        .into_iter()
...
#[test]
fn native_piper_roundtrip_via_whisper() {
    let (Some(dir), Some(wd)) = (piper_dir(), whisper_dir()) else {
        eprintln!("skip: set RLX_PIPER_DIR (+ rlx-split bundle) + RLX_WHISPER_DIR");
        return;
    };
    let tts = NativeVits::load(&dir, Device::Cpu).expect("load native piper");
