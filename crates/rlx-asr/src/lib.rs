...
//!
//! The Rust CLI uses the folded encoder when `encoder.*` tensors are in the pack;
//! Python `tools/e2e_native_whole.py` runs the same folded CTC path from the same pack.

pub mod beam;
pub mod effective_decoder;
pub mod encoder;
pub mod env;
pub mod folded_encoder;
pub mod frontend;
pub mod gguf_io;
pub mod k_codebook;
...
pub use env::{AsrPaths, EncoderMode, asr_dir, asr_dir_env, encoder_mode, timing};
pub use gguf_io::{
...
pub use native_encoder::{NativeEncoder, StreamingState};
pub use pipeline::{AsrSession, StreamingAsr, TranscribeOpts, Transcript};
