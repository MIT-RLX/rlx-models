...
use crate::{PorcupineEngine, PorcupineWeights};
...
pub fn run(args: &[String]) -> Result<()> {
    ...
    let mut keyword = "porcupine".to_string();
    let mut i = 0;
    while i < args.len() {
        match args[i].as_str() {
            "--wav" => {
            ...
            "-h" | "--help" => {
                println!(
                    "rlx-porcupine --wav PATH [--weights model.safetensors] [--device cpu|metal|…|all]"
                );
                return Ok(());
            }
    ...
    let w = if let Some(p) = weights {
        PorcupineWeights::load(&p)?
    } else {
        PorcupineWeights::stub(&keyword)
    };
    let cfg = WakeConfig {
        threshold,
