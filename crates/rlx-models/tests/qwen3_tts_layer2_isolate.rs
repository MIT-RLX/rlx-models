...
//! Layer-2 forward from HF layer-1 hidden (`hf_h1_full.json`).
...
fn layer2_from_hf_layer1_hidden() {
    ...
    let hf_l2: Vec<f32> = layers["layer_last"][2]
        .as_array()
        .unwrap()
        .iter()
        .map(|v| v.as_f64().unwrap() as f32)
    ...
    let out = eager.forward_layer(2, x.view(), &positions, 0).unwrap();
    let native: Vec<f32> = out.row(out.nrows() - 1).iter().copied().collect();
    let mut max_d = 0f32;
    for (a, b) in native.iter().zip(hf_l2.iter()) {
        max_d = max_d.max((a - b).abs());
    }
    eprintln!("layer2 from hf-h1 max_abs={max_d}");
    assert!(max_d < 0.01, "layer2 isolated diverged (max_abs={max_d})");
}
