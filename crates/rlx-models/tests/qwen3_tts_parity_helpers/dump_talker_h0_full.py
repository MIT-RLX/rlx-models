...
"""Dump full hidden states after talker layer 0 (HF)."""
...
def main() -> None:
    ...
    hk = talker_dec.layers[0].register_forward_hook(hook)
    with torch.no_grad():
    ...
    with open(args.out, "w", encoding="utf-8") as f:
        json.dump(
            {"seq": seq, "hidden": h0[0].reshape(-1).cpu().tolist(), "last": h0[0, -1].cpu().tolist()},
            f,
        )
