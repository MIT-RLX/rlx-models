...
//
// CPU vs every available backend on a MiniCPM5-shaped synthetic llama32 graph.
//
//   cargo test -p rlx-models --test minicpm5_backend_parity --features all-backends --release
...
fn assert_backend_matches_cpu(name: &str, device: Device) {
    if !rlx_runtime::is_available(device) {
        eprintln!("skip minicpm5 {name}: {device:?} not available");
        return;
    }
    let cpu = run_last_logits(Device::Cpu);
    let other = run_last_logits(device);
    let c = cosine(&cpu, &other);
    eprintln!("minicpm5 cpu vs {name} cosine={c:.8}");
    assert!(c > 0.99, "minicpm5 cpu vs {name} cosine {c}");
}
