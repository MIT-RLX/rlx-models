...
//
// Real TinyLlama-1.1B GGUF: packed prefill on CPU / Metal / MLX / CUDA / WGPU.
//
// ```sh
// just fetch-tinyllama-gguf Q4_K_M
// just test-tinyllama-gguf-backends
//
// RLX_TINYLLAMA_GGUF_Q4_K_M=/path/TinyLlama-1.1B-Chat-v1.0-Q4_K_M.gguf \
//   cargo test -p rlx-models --test tinyllama_backend_gguf_check --features all-backends,tinyllama,llama32 --release -- --nocapture
// ```

#![allow(dead_code)]

use rlx_runtime::Device;
use rlx_tinyllama::{TINYLLAMA_GGUF_FILES, TinyLlamaRunner};
use std::panic::{AssertUnwindSafe, catch_unwind};
use std::path::{Path, PathBuf};
use std::time::Instant;

fn gguf_dir() -> PathBuf {
    std::env::var("RLX_TINYLLAMA_GGUF_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("/tmp/rlx-weights/TinyLlama-1.1B-GGUF"))
}

fn gguf_path_for_quant(quant: &str) -> Option<PathBuf> {
    let env_key = format!("RLX_TINYLLAMA_GGUF_{quant}");
    ...
    if let Ok(p) = std::env::var("RLX_TINYLLAMA_GGUF_PATH") {
        let path = PathBuf::from(p);
        if path.is_file() {
            return Some(path);
        }
...
fn assert_backend_matches_cpu(label: &str, cpu_logits: &[f32], backend_logits: &[f32], ms: f64) {
    ...
    eprintln!(
        "tinyllama gguf {label}: cos={cos:.6} top3={backend_top3:?} cpu_top3={cpu_top3:?} {ms:.1}ms"
    );
...
fn gguf_cpu_reference(quant: &str) -> Option<(PathBuf, Vec<f32>)> {
    ...
    eprintln!(
        "tinyllama gguf cpu [{quant}]: n_vocab={} top3={:?} {ms:.1}ms",
        logits.len(),
...
fn run_q4_on_device(label: &str, device: Device) {
    ...
    let Some((path, cpu_logits)) = cpu_ref else {
        eprintln!("skip {label}: just fetch-tinyllama-gguf Q4_K_M");
        return;
    };
    if device != Device::Cpu && !rlx_runtime::is_available(device) {
        eprintln!("skip tinyllama gguf {label}: unavailable");
        return;
    }
    ...
    match outcome {
        ...
        Err(_) => {
            eprintln!("tinyllama gguf {label}: panic during packed prefill (backend gap)");
        }
    }
}

#[test]
fn tinyllama_gguf_q4_k_m_cpu() {
    let Some((path, _)) = gguf_cpu_reference("Q4_K_M") else {
        eprintln!("skip: just fetch-tinyllama-gguf Q4_K_M");
        return;
    };
...
#[test]
fn tinyllama_gguf_q6_k_cpu() {
    let Some((path, cpu)) = gguf_cpu_reference("Q6_K") else {
        eprintln!("skip Q6_K: just fetch-tinyllama-gguf Q6_K");
        return;
    };
    let (other, ms) = run_predict(&path, Device::Cpu);
    assert_backend_matches_cpu("cpu/Q6_K", &cpu, &other, ms);
}
