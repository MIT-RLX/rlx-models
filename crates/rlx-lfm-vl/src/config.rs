...
//! LFM2.5-VL vision-tower configuration. Maps to the `vision_config`
//! block of the HF LFM2-VL `config.json` and corresponding `vision.*`
//! keys in LiquidAI's mmproj GGUF.
...
pub struct LfmVlVisionConfig {
    ...
    /// LM hidden size — output dim of `multi_modal_projector.linear_2`.
    pub projector_output_dim: usize,
}

impl LfmVlVisionConfig {
...
impl LfmVlVisionConfig {
    pub fn from_hf_config_json(path: &Path) -> Result<Self> {
        let raw =
            std::fs::read_to_string(path).map_err(|e| anyhow!("rlx-lfm-vl: read {path:?}: {e}"))?;
        let cfg: HfTop =
            serde_json::from_str(&raw).map_err(|e| anyhow!("rlx-lfm-vl: parse {path:?}: {e}"))?;
        Ok(Self {
