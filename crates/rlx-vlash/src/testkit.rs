...
pub fn synth_weights(cfg: &VlashConfig) -> WeightMap {
    ...
    let mut seed = 1usize;
    let mut put = |t: &mut HashMap<String, (Vec<f32>, Vec<usize>)>, key: &str, shape: Vec<usize>| {
    ...
    let v = &cfg.vision;
    // Vision embeddings.
    put(&mut t, "vision.embeddings.patch_embedding.weight", vec![v.width, 3, v.patch_size, v.patch_size]);
    put(&mut t, "vision.embeddings.patch_embedding.bias", vec![v.width]);
    put(&mut t, "vision.embeddings.position_embedding.weight", vec![v.num_patches(), v.width]);
    ...
    for i in 0..v.layers {
        ...
        for pr in ["q_proj", "k_proj", "v_proj", "out_proj"] {
            put(&mut t, &format!("{p}.self_attn.{pr}.weight"), vec![v.width, v.width]);
            put(&mut t, &format!("{p}.self_attn.{pr}.bias"), vec![v.width]);
        }
        put(&mut t, &format!("{p}.mlp.fc1.weight"), vec![v.intermediate, v.width]);
        put(&mut t, &format!("{p}.mlp.fc1.bias"), vec![v.intermediate]);
        put(&mut t, &format!("{p}.mlp.fc2.weight"), vec![v.width, v.intermediate]);
    ...
    put(&mut t, "vision.projector.weight", vec![v.projection_dim, v.width]);
    put(&mut t, "vision.projector.bias", vec![v.projection_dim]);

    // Gemma stacks.
    let gemma_layer = |t: &mut HashMap<String, (Vec<f32>, Vec<usize>)>,
        ...
        for i in 0..g.layers {
            ...
            for ln in ["input_layernorm", "post_attention_layernorm"] {
                if g.use_adarms {
                    put(t, &format!("{p}.{ln}.dense.weight"), vec![3 * g.hidden, g.hidden]);
                    put(t, &format!("{p}.{ln}.dense.bias"), vec![3 * g.hidden]);
                } else {
                    put(t, &format!("{p}.{ln}.weight"), vec![g.hidden]);
                }
            }
            put(t, &format!("{p}.self_attn.q_proj.weight"), vec![g.attn_dim(), g.hidden]);
            put(t, &format!("{p}.self_attn.k_proj.weight"), vec![g.kv_dim(), g.hidden]);
            put(t, &format!("{p}.self_attn.v_proj.weight"), vec![g.kv_dim(), g.hidden]);
            put(t, &format!("{p}.self_attn.o_proj.weight"), vec![g.hidden, g.attn_dim()]);
            put(t, &format!("{p}.mlp.gate_proj.weight"), vec![g.intermediate, g.hidden]);
            put(t, &format!("{p}.mlp.up_proj.weight"), vec![g.intermediate, g.hidden]);
            put(t, &format!("{p}.mlp.down_proj.weight"), vec![g.hidden, g.intermediate]);
        }
        // final norm
        if g.use_adarms {
            put(t, &format!("{prefix}.norm.dense.weight"), vec![3 * g.hidden, g.hidden]);
    ...
    put(&mut t, "suffix.action_in_proj.weight", vec![d, cfg.max_action_dim]);
    put(&mut t, "suffix.action_in_proj.bias", vec![d]);
    match cfg.variant {
        VlashVariant::Pi0 => {
            put(&mut t, "suffix.state_proj.weight", vec![d, cfg.max_state_dim]);
    ...
    put(&mut t, "action_out_proj.weight", vec![cfg.max_action_dim, d]);
