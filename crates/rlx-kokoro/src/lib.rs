...
//! shares the espeak-ng → IPA frontend and the 3-input graph interface
//! (`input_ids` / `style` / `speed`) with [`rlx_kittentts`], from which the
//! phonemizer and text preprocessor are reused.
//!
//! ## Quick start
//!
//! ```no_run
...
//! ## Backends
//!
//! Inference runs through the ort-free **`native`** RLX path: the monolithic
//! graph is split into `encoder.onnx` + `decoder_raw.onnx` (via
//! `scripts/split_kokoro.py`) with the data-dependent length regulator and ISTFT
//! overlap-add done in Rust; both subgraphs import through `rlx-onnx-import` →
//! rlx-ir and run on any RLX backend (cpu / metal / mlx / wgpu, plus cuda / rocm
//! / coreml / vulkan via the matching features). See [`native::NativeKokoro`].
//! The split decomposition is verified bit-exact against the monolithic model.
...
pub use model::{MIN_AUDIBLE_PEAK, peak_amplitude, write_wav};
#[cfg(feature = "native")]
