...
//! Host-side text token + position embedding.
//!
//! Performed on the CPU (like the vision patch stem) so the text graph is a
//! pure float pipeline — no in-graph integer `Gather`. `hidden[i] =
//! token_embedding[ids[i]] + position_embedding[i]`.
...
pub struct TextEmbedWeights {
    ...
    /// `position_embedding.weight` `[ctx · width]`.
...
pub(crate) fn extract_text_embed_weights(
    ...
    let (token_embedding, tok_shape) =
        weights.take("text_model.embeddings.token_embedding.weight")?;
    ensure!(
    ...
    let (positional, pos_shape) =
        weights.take("text_model.embeddings.position_embedding.weight")?;
    ensure!(
        positional.len() == ctx * width,
        "position_embedding length {} != ctx*width ({ctx}*{width}); shape={pos_shape:?}",
        positional.len()
    );
