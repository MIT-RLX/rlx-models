...
use crate::{VoxrtEngine, VoxrtWeights};
...
pub fn run(args: &[String]) -> Result<()> {
    ...
    let mut keyword = "hey assistant".to_string();
    let mut i = 0;
    while i < args.len() {
        match args[i].as_str() {
            "--wav" => {
            ...
            "-h" | "--help" => {
                println!(
                    "rlx-voxrt --wav PATH [--weights model.safetensors] [--device cpu|metal|…|all]"
                );
                return Ok(());
            }
    ...
    for device in devices {
        ...
        let mut eng = VoxrtEngine::new(w.clone(), cfg.clone()).with_device_label(label);
        let steps = score_wav(&mut eng, &pcm)?;
        println!(
            "engine=voxrt keyword={} device={} peak={:.4} fires={}",
            eng.keyword(),
            eng.device_label(),
            peak_score(&steps),
