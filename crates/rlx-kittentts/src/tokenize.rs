...
/// Number of vocabulary IDs between the start/end pad tokens (0).
pub fn ipa_content_len(ipa: &str) -> usize {
    ipa_to_ids(ipa).len().saturating_sub(2)
}

/// Style row index for voice embedding lookup (phoneme token count, min 1).
pub fn ipa_style_index(ipa: &str) -> usize {
    ipa_content_len(ipa).max(1)
}

/// Style row index from UTF-8 byte length (kittentts-rs text / espeak path).
