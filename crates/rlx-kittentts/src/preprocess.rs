...
//!
//! Converts raw input text (with numbers, symbols, abbreviations, etc.)
        ...
        .replace_all(text, |caps: &Captures| {
            ...
            let denom_word = match den {
                ...
                _ => {
                    ...
                    if num != 1 { format!("{}s", ord) } else { ord }
                }
            };
...
pub fn expand_contractions(text: &str) -> String {
    ...
    let text = RE_CONTRACTION_WONT
        .replace_all(&text, "will not")
        .into_owned();
    let text = RE_CONTRACTION_SHANT
        .replace_all(&text, "shall not")
        .into_owned();
    let text = RE_CONTRACTION_AINT
        .replace_all(&text, "is not")
        .into_owned();
    let text = RE_CONTRACTION_LETS
        .replace_all(&text, "let us")
        .into_owned();
