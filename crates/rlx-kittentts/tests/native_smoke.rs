...
fn native_infer_smoke() {
    ...
    if assets::find_rlx_bundle(&weights).is_none() {
        eprintln!(
            "skip native_infer_smoke: no rlx_bundle under {}",
            weights.display()
        );
        return;
    }
...
fn native_long_sentence_smoke() {
    ...
    if assets::find_rlx_bundle(&weights).is_none() {
        eprintln!("skip native_long_sentence_smoke: no rlx_bundle");
        return;
    }
    let Some(voices) = voices_npz() else {
    ...
    let audio = tts
        ...
        .expect("infer long");
    assert_audible(&audio, 15_000);
    eprintln!("native long smoke: {} samples", audio.len());
}
