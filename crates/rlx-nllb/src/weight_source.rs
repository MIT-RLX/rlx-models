...
//!
//! Encoder / decoder graphs (recompiled per sequence length) all read from one
//! persistent [`WeightMap`], so we cannot consume tensors with the default
//! destructive `take`. This source clones instead, applying the same 2-D
//! transpose semantics as `WeightMap::take_transposed`.
