...
//! Mel frontend aligned with openWakeWord chunking (16 kHz, 80 ms chunks).

use crate::SAMPLE_RATE_16K;

/// openWakeWord streaming chunk size (80 ms @ 16 kHz).
pub const OWW_CHUNK_SAMPLES: usize = 1280;
...
impl Default for MelConfig {
    fn default() -> Self {
        // Matches common OWW / torchaudio MelSpectrogram settings used upstream.
        Self {
...
/// Streaming mel extractor with OWW-style `(x / 10) + 2` normalization.
...
impl MelFrontend {
    ...
    /// Push PCM; returns mel frames as row-major `[n_frames * n_mels]` (frame-major).
...
/// Real FFT power spectrum via naive DFT (n_fft is small: 400).
fn rfft_power(x: &[f32]) -> Vec<f32> {
    ...
    for k in 0..n_freqs {
        let mut re = 0.0f32;
        let mut im = 0.0f32;
        let ang0 = -std::f32::consts::TAU * k as f32 / n as f32;
...
mod tests {
    ...
    #[test]
    fn chunk_produces_frames() {
        let mut mel = MelFrontend::new(MelConfig::default());
        let pcm = vec![0.01f32; OWW_CHUNK_SAMPLES];
        let frames = mel.push(&pcm);
        assert!(frames.len() >= mel.n_mels());
