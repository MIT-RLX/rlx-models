...
fn main() -> anyhow::Result<()> {
    ...
    let dir = PathBuf::from(
        a.next()
            .unwrap_or_else(|| "weights/tts/supertonic-3".into()),
    );
    ...
    println!(
        "device={device:?} ep={} voice={voice} steps={steps} iters={iters}\n",
        tts.ort_ep()
    );
    ...
    if !warm.is_empty() {
        println!(
            "\nwarm mean: {:.1}× real-time on {device:?}",
            warm.iter().sum::<f32>() / warm.len() as f32
        );
    }
    Ok(())
}
