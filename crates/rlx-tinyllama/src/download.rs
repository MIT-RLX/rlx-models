...
//! Hugging Face Hub download for [TinyLlama/TinyLlama-1.1B-Chat-v1.0](https://huggingface.co/TinyLlama/TinyLlama-1.1B-Chat-v1.0).
//!
...
use crate::{HF_MODEL_ID_1_1B, HF_MODEL_ID_GGUF, TINYLLAMA_GGUF_FILES};
...
/// Download TinyLlama-1.1B-Chat into the HF cache; returns the snapshot directory.
#[cfg(feature = "hf-download")]
pub fn download_tinyllama_1_1b(cache_dir: &Path) -> Result<PathBuf> {
    let api = hf_hub::api::sync::ApiBuilder::new()
        .with_cache_dir(cache_dir.to_path_buf())
        .build()
        .context("hf_hub ApiBuilder")?;
    let repo = api.model(HF_MODEL_ID_1_1B.to_string());
...
fn weight_shard_names(repo: &hf_hub::api::sync::ApiRepo) -> Result<Vec<String>> {
    ...
    if repo.get("model-00001-of-00003.safetensors").is_ok() {
        return Ok(vec![
            "model-00001-of-00003.safetensors".into(),
            "model-00002-of-00003.safetensors".into(),
            "model-00003-of-00003.safetensors".into(),
        ]);
    }
    if repo.get("model.safetensors").is_ok() {
        return Ok(vec!["model.safetensors".into()]);
    }
    bail!("no safetensors shards found for {HF_MODEL_ID_1_1B}")
}

/// Copy or symlink checkpoint files from the HF cache snapshot into `dest`.
#[cfg(feature = "hf-download")]
pub fn materialize_tinyllama_1_1b(snapshot: &Path, dest: &Path) -> Result<PathBuf> {
...
fn list_weight_files(snapshot: &Path) -> Result<Vec<String>> {
    ...
    for candidate in [
        ...
        if snapshot.join(candidate).is_file() {
            if candidate.contains("00001-of-00003") {
                return Ok(vec![
                    "model-00001-of-00003.safetensors".into(),
                    "model-00002-of-00003.safetensors".into(),
                    "model-00003-of-00003.safetensors".into(),
...
#[cfg(feature = "hf-download")]
pub fn download_tinyllama_gguf(cache_dir: &Path, quant_label: &str) -> Result<PathBuf> {
    let filename = TINYLLAMA_GGUF_FILES
        .iter()
        .find(|(label, _)| *label == quant_label)
        .map(|(_, f)| *f)
        .ok_or_else(|| {
            let names: Vec<_> = TINYLLAMA_GGUF_FILES.iter().map(|(l, _)| *l).collect();
...
#[cfg(feature = "hf-download")]
pub fn materialize_tinyllama_gguf(src_gguf: &Path, dest_dir: &Path) -> Result<PathBuf> {
...
#[cfg(feature = "hf-download")]
pub fn fetch_tinyllama_gguf(
    cache_dir: &Path,
    dest_dir: &Path,
    quant_label: &str,
) -> Result<PathBuf> {
    let src = download_tinyllama_gguf(cache_dir, quant_label)?;
    materialize_tinyllama_gguf(&src, dest_dir)
}

/// Download into the HF cache, then materialize under `dest` (flat layout for RLX).
#[cfg(feature = "hf-download")]
pub fn fetch_tinyllama_1_1b(cache_dir: &Path, dest: &Path) -> Result<PathBuf> {
    let snapshot = download_tinyllama_1_1b(cache_dir)?;
    materialize_tinyllama_1_1b(&snapshot, dest)
}

#[cfg(not(feature = "hf-download"))]
pub fn download_tinyllama_1_1b(_cache_dir: &Path) -> Result<PathBuf> {
    bail!(
        "HF download requires the `hf-download` feature — rebuild with \
         `cargo build -p rlx-tinyllama --features hf-download`"
    )
}

#[cfg(not(feature = "hf-download"))]
pub fn materialize_tinyllama_1_1b(_snapshot: &Path, _dest: &Path) -> Result<PathBuf> {
    download_tinyllama_1_1b(Path::new("."))
}

#[cfg(not(feature = "hf-download"))]
pub fn fetch_tinyllama_1_1b(_cache_dir: &Path, _dest: &Path) -> Result<PathBuf> {
    download_tinyllama_1_1b(_cache_dir)
}

#[cfg(not(feature = "hf-download"))]
pub fn download_tinyllama_gguf(_cache_dir: &Path, _quant_label: &str) -> Result<PathBuf> {
    bail!("HF download requires the `hf-download` feature")
}

#[cfg(not(feature = "hf-download"))]
pub fn materialize_tinyllama_gguf(_src: &Path, _dest_dir: &Path) -> Result<PathBuf> {
    download_tinyllama_gguf(Path::new("."), "Q4_K_M")
}
