...
//! Small host-side tensor kernels shared by native model bring-up paths
//! (SAM3, FLUX.2, V-JEPA2). Relocated from the former `rlx-tensor-host` crate;
//! the math is unchanged.
//!
//! Compute-heavy ops go through `rlx_cpu::blas` (Accelerate/OpenBLAS, with a
//! scalar fallback) instead of naive Rust loops.
