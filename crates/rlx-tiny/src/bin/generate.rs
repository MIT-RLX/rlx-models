//! Generate stories from a trained rlx-tiny checkpoint.
//!
//! ```text
...
use rlx_tiny::checkpoint;
use rlx_tiny::sample::{GenOptions, generate};

fn main() -> Result<()> {
    let mut model = PathBuf::from("weights/tiny/tinystories.rlxts");
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            ...
            "-h" | "--help" => {
                println!(
                    "rlx-tiny-generate [--model FILE] [--prompt STR] [--tokens N]\n\
