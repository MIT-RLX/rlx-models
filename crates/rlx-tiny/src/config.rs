...
pub struct GptConfig {
    ...
    /// Residual multi-stage VQ depth for codebook weight-synthesis: `W = Σ_s
    /// codebook_s[idx_s]`. Each stage adds a tiny u8 index table + L1-resident
    /// codebook — more in-kernel compute, ~no extra DRAM weight bytes. `1` = a
    /// single codebook. Multiplies codebook DOF by `synth_stages`.
    pub synth_stages: usize,
    /// Low-rank dense correction rank for `W += A·Bᵀ` (`0` disables). Recovers
    /// degrees of freedom a fixed-assignment codebook structurally can't reach.
    pub lora_rank: usize,
}
...
impl GptConfig {
    ...
    ///
    /// NOTE: this counts the *dense-equivalent* weight matrices. The actual
    /// trainable parameters are far fewer — every `[k,n]` weight is replaced by
    /// codebook weight-synthesis (`synth_stages` codebooks of `NE·ED` numbers)
    /// plus an optional rank-`lora_rank` correction — so this over-counts and is
    /// only a rough capacity yardstick, not a compressed footprint.
    ...
    pub fn check(&self) -> Result<(), String> {
        ...
        if self.synth_stages < 1 {
            return Err("synth_stages must be >= 1 (1 = a single codebook)".into());
        }
        Ok(())
    }
}
