...
// V3 adds two capacity fields to the header (`synth_stages`, `lora_rank`, both
// u32, right after `n_embd`) so a checkpoint trained with swept codebook depth /
// LoRA rank rebuilds the matching graph on reload. V1/V2 files (no such fields)
// default to the historical 2 / 8.
const MAGIC_V1: &[u8; 8] = b"RLXTS1\0\0";
const MAGIC_V2: &[u8; 8] = b"RLXTS2\0\0";
const MAGIC: &[u8; 8] = b"RLXTS3\0\0";
/// Header defaults for pre-V3 checkpoints (the values these knobs were hard-coded to).
const LEGACY_SYNTH_STAGES: usize = 2;
const LEGACY_LORA_RANK: usize = 8;
...
pub fn load(path: &Path) -> Result<(GptConfig, Vec<(String, Vec<f32>)>, Option<Bpe>)> {
    ...
    let v3 = &magic == MAGIC;
    // V2 and V3 both carry the trailing tokenizer section.
    let has_tokenizer = v3 || &magic == MAGIC_V2;
    if !has_tokenizer && &magic != MAGIC_V1 {
        bail!("{}: not an rlx-tiny checkpoint", path.display());
    }
    ...
    // V3 adds synth_stages + lora_rank; older files default to the historical values.
    let (synth_stages, lora_rank) = if v3 {
        (read_u32(&mut r)? as usize, read_u32(&mut r)? as usize)
    } else {
        (LEGACY_SYNTH_STAGES, LEGACY_LORA_RANK)
    };
    let cfg = GptConfig {
        vocab,
        block_size,
    ...
    // V2/V3 tokenizer section (absent in V1 → byte-level).
    let bpe = if has_tokenizer {
