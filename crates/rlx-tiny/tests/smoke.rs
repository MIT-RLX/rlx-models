...
use rlx_tensor::{Device, LrSchedule};

use rlx_tiny::config::GptConfig;
use rlx_tiny::data::{Batcher, Tokens};
use rlx_tiny::sample::{GenOptions, generate};
use rlx_tiny::{HybridOptimizer, Rng, model};
...
fn gpt_learns_and_generates_on_cpu() {
    ...
    let mut m = model::init(model::build(&cfg, cfg.batch, true), &cfg, 42);
