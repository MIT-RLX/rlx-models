// RLX models — model reference parsing.
// Copyright (C) 2026 Eugene Hauptmann, Nataliya Kosmyna.
//
// SPDX-License-Identifier: GPL-3.0-only

//! Parsing and formatting of model references.
//!
//! A [`ModelRef`] identifies a HuggingFace repository plus an optional
//! revision and quantization selector. Accepted forms:
//!
//! - `org/repo`
//! - `org/repo:Q4_K_M` (quant selector)
//! - `org/repo@abc123` (revision)
//! - `org/repo@abc123:Q4_K_M` / `org/repo:Q4_K_M@abc123` (both, either order)
//! - `https://huggingface.co/org/repo[:SELECTOR]`
...
/// A parsed model reference: `org/repo` with optional `@revision` and
/// `:selector` (quantization) parts.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ModelRef {
    /// The `org/repo` identifier.
    pub repo: String,
    /// Optional revision (git ref / commit sha). `None` means "main".
    pub revision: Option<String>,
    /// Optional quantization selector, e.g. `Q4_K_M`, `BF16`, `UD-Q4_K_XL`.
    pub selector: Option<String>,
}

impl ModelRef {
    /// Parse a user-supplied model reference string.
    pub fn parse(input: &str) -> Result<Self, ModelRefParseError> {
        parse_model_ref(input)
    }

    /// The revision to use, defaulting to `main` when unspecified.
    pub fn revision_or_main(&self) -> &str {
        self.revision.as_deref().unwrap_or("main")
    }

    /// The canonical display id, round-trippable through [`ModelRef::parse`].
...
/// Error returned when a string cannot be parsed as a [`ModelRef`].
...
/// Parse a model reference. See the module docs for accepted forms.
...
/// Format a `repo`/`revision`/`selector` triple into the canonical string.
...
/// Format a canonical `repo@revision/file` reference.
pub fn format_canonical_ref(repo: &str, revision: &str, file: &str) -> String {
    format!("{repo}@{revision}/{file}")
}

/// Extract the quantization selector token from a GGUF filename, if present.
///
/// Handles common real-world casings (`...-Q4_K_M.gguf`, `....q4_k_m.gguf`)
/// and directory-prefixed layouts (`BF16/model-BF16-00001-of-00002.gguf`).
pub fn quant_selector_from_gguf_file(file: &str) -> Option<String> {
    ...
    // Markers are matched case-insensitively; the returned selector is sliced
    // from the *original* stem so the model's preferred casing survives.
...
/// Collapse a (possibly split/sharded) GGUF filename to its distribution id.
...
/// Whether a token looks like a quantization selector (`Q*`, `IQ*`, `UD-*`,
/// `BF16`, `F16`, `F32`).
...
/// Whether a GGUF file path matches a given quant selector.
...
/// Info about a split (multi-part) GGUF shard filename.
...
/// Parse `<prefix>-NNNNN-of-NNNNN.gguf` shard info, if the filename matches.
...
mod tests {
    ...
    fn parses_public_model_refs() {
        ...
        assert_eq!(
            parse_model_ref("org/repo").unwrap(),
            ModelRef {
                repo: "org/repo".to_string(),
                revision: None,
                selector: None,
        ...
        // Selector-before-revision normalizes to the canonical order.
        assert_eq!(
    ...
    fn revision_defaults_to_main() {
        assert_eq!(
            parse_model_ref("org/repo").unwrap().revision_or_main(),
            "main"
        );
        assert_eq!(
            parse_model_ref("org/repo@dev").unwrap().revision_or_main(),
            "dev"
        );
    }

    #[test]
    fn rejects_exact_file_refs() {
    ...
    fn extracts_quant_selectors_from_gguf_files() {
        ...
        assert_eq!(quant_selector_from_gguf_file("config.json"), None);
    }
