...
onnx.save(model, '/tmp/k_f32_cmp.onnx')
...
sess = ort.InferenceSession('/tmp/k_f32_cmp.onnx', providers=['CPUExecutionProvider'])
...
fn f32_chain_before_query1_qmatmul() {
    ...
    let ort_names: Vec<&str> = resolved.iter().map(|(_, _, o)| *o).collect();
    let ort = ort_tensors(&ort_names);
    const SPOT: usize = 3576;
    eprintln!("=== f32 before QMatMul (one coexec run) ===");
    ...
    for (i, (_, hir, ort_name)) in resolved.iter().enumerate() {
        ...
        let short = hir
            ...
            .collect::<Vec<_>>()
            .join("/");
        eprintln!("{short:45} max={max:.4} idx={idx} spot3576={spot:.4}");
        if max > worst.0 {
            worst = (max, short);
        }
    }
    eprintln!("worst: {} max={:.4}", worst.1, worst.0);
    assert!(
        worst.0 < 0.5,
        "f32 chain before query_1 should track ORT within 0.5, worst {} = {:.4}",
        worst.1,
        worst.0
    );
}
