...
//! Isolated Tanh + GELU probes vs ORT (run gelu first to warm arena).
...
onnx.save(model, '/tmp/k_gelu_tanh.onnx')
...
sess = ort.InferenceSession('/tmp/k_gelu_tanh.onnx', providers=['CPUExecutionProvider'])
...
const PROBES: &[(&str, &str)] = &[
    ...
    (
        "/bert/encoder/albert_layer_groups.0/albert_layers.0/activation/Pow",
        "/bert/encoder/albert_layer_groups.0/albert_layers.0/activation/Pow_output_0",
    ),
    (
        "/bert/encoder/albert_layer_groups.0/albert_layers.0/activation/Add",
        "/bert/encoder/albert_layer_groups.0/albert_layers.0/activation/Add_output_0",
    ),
    (
        "/bert/encoder/albert_layer_groups.0/albert_layers.0/activation/Mul_2",
        "/bert/encoder/albert_layer_groups.0/albert_layers.0/activation/Mul_2_output_0",
    ),
    (
        "/bert/encoder/albert_layer_groups.0/albert_layers.0/activation/Tanh",
        "/bert/encoder/albert_layer_groups.0/albert_layers.0/activation/Tanh_output_0",
    ),
    (
...
fn gelu_subgraph_f32_vs_ort() {
    ...
    eprintln!("=== GELU subgraph (warmup + sequential single probes) ===");
    let mut worst = (0.0f32, String::new(), 0usize);
    for (hir, ort_name) in WARMUP.iter().chain(PROBES.iter()) {
        let id = import
            .hir
            .nodes()
            .iter()
        ...
        let short = hir
            ...
            .join("/");
        eprintln!(
            "{short:45} max={max:.4} idx={idx} nat={} ort={}",
            nat[idx], o[idx]
        );
        for spot in [1272usize, 2265, 6140, 8522] {
            if nat.len() > spot && o.len() > spot {
                eprintln!(
                    "  @{spot} d={:.4} nat={} ort={}",
                    (nat[spot] - o[spot]).abs(),
                    nat[spot],
