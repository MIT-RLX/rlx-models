...
//!
//! Submodules: encoder–decoder, batch/device sweeps (+ HTML reports),
//! Welch-peaks, end-to-end pipelines, and fusion-phase breakdowns.

pub mod e2e;
pub mod e2e_html;
pub mod encdec;
#[cfg(feature = "dev")]
pub mod fusion_phases;
pub mod sweep;
pub mod sweep_html;
pub mod welch_peaks;
...
use crate::model::weights::{EncDecWeights, WeightStore, load_safetensors};
...
pub fn bench_all_dir(
    ...
    let (signal, spectrum_interleaved, rlx_input, rlx_input_name) = if dir.is_forward() {
        ...
        let block = crate::exec::rlx_fft::interleaved_to_block(&spectrum, batch, n_fft);
    ...
    let mut rlx_exec = crate::exec::rlx_fft::compile_rlx_fft(&cfg, dir, device)?;
    ...
    let rlx_pred = crate::model::reference::block_to_interleaved(&rlx_out[0], batch, n_fft);
...
mod tests {
    ...
    fn rlx_fft_graph_builds() {
        use crate::exec::rlx_fft::{build_rlx_fft_forward_graph, build_rlx_fft_inverse_graph};
