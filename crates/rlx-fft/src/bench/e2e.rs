...
use crate::distill::compile::{CompiledDistilledMel, compile_distilled_mel};
use crate::distill::fused::pick_fused_deploy_device;
use crate::distill::model::DistilledFftModel;
use crate::exec::rlx_fft::{compile_rlx_fft, rlx_fft_forward};
use crate::learned::compile::{
    CompiledLearnedMel, compile_learned_mel, default_hard_threshold, window_batch as compile_window,
};
use crate::learned::model::{FastLearnedFftModel, pipeline_max_err, ref_spectrum_batch, ref_welch};
use crate::model::butterfly::butterfly_forward_real_batch;
use crate::model::config::FftLearnConfig;
use crate::model::config::TransformDir;
use crate::model::pruned::DEFAULT_GATE_THRESHOLD;
use crate::model::q8::Q8Twiddles;
use crate::model::reference::fft_real_batch;
use crate::model::twiddle::exact_twiddles;
use crate::spectral::mel::{hann_window, log_mel_from_spectrum_batch, log_mel_from_windowed_batch};
use crate::spectral::peak::{
    DEFAULT_PEAK_K, WelchPeakParams, WelchPeaksScratch, welch_peaks_rustfft,
};
...
use crate::spectral::welch_peaks_picker::AutoWelchPeaks;
...
pub struct E2eBatchTrainMeta {
    pub batch: usize,
    pub teacher: Option<crate::train::e2e::E2eTrainReport>,
    pub distill: Option<crate::train::distill::DistillTrainReport>,
}
...
pub struct E2eBenchInputs<'a> {
    ...
    pub distilled_ternary: Option<&'a crate::distill::ternary_model::DistilledTernaryFftModel>,
...
pub fn run_e2e_bench(inputs: &E2eBenchInputs<'_>) -> Result<E2eBenchReport> {
    ...
    let _gates = crate::model::pruned::init_gates(inputs.n_fft);
    ...
    let mut compiled_distilled: Option<crate::distill::compile::CompiledDistilledMel> = None;
    let mut compiled_distilled_ternary: Option<
        crate::distill::ternary_compile::CompiledDistilledTernaryMel,
    > = None;
...
fn bench_backend(
    ...
    compiled_distilled: &mut Option<crate::distill::compile::CompiledDistilledMel>,
    ...
    let mut run = || -> Result<Vec<f32>> {
        match (pipeline, backend) {
            ...
            (E2ePipeline::Welch, E2eBackend::ButterflyEager) => {
                crate::spectral::welch::welch_butterfly(
                    welch_signal,
                    tw,
                    inputs.batch,
                    welch_params,
...
fn welch_peaks_for_backend(
    ...
    match backend {
        ...
        E2eBackend::ButterflyEager => {
            ...
            let psd = crate::spectral::welch::welch_butterfly(
                peak_fast_buf,
                tw,
                inputs.batch,
                peak_params.welch,
            )?;
            Ok(crate::spectral::peak::peaks_from_psd_batch(
                &psd,
                inputs.batch,
                peak_params.n_bins(),
                peak_params.k,
...
fn compiled_distilled_ternary_mel(
    cache: &mut Option<crate::distill::ternary_compile::CompiledDistilledTernaryMel>,
    inputs: &E2eBenchInputs<'_>,
    model: &crate::distill::ternary_model::DistilledTernaryFftModel,
    signal: &[f32],
) -> Result<Vec<f32>> {
...
fn ensure_compiled_distilled_ternary<'a>(
    cache: &'a mut Option<crate::distill::ternary_compile::CompiledDistilledTernaryMel>,
    inputs: &E2eBenchInputs<'_>,
    model: &crate::distill::ternary_model::DistilledTernaryFftModel,
) -> Result<&'a mut crate::distill::ternary_compile::CompiledDistilledTernaryMel> {
    if cache.is_none() {
        let cfg = FftLearnConfig::new(inputs.n_fft, inputs.batch)?;
        let deploy = pick_fused_deploy_device(inputs.batch, inputs.n_fft, inputs.device);
        *cache = Some(
            crate::distill::ternary_compile::compile_distilled_ternary_mel(model, &cfg, deploy)?,
        );
    }
...
fn rlx_mel(
    ...
    let filters = crate::spectral::mel::mel_filterbank(inputs.n_fft, n_mels, sr);
    crate::spectral::mel::log_mel_from_spectrum_batch(
        &spec,
        &filters,
        inputs.batch,
        inputs.n_fft,
...
fn rlx_welch(
    ...
    let window = crate::spectral::welch::hann_window(params.n_fft);
    ...
    Ok(crate::spectral::welch::average_welch_psd(
        &spec,
        inputs.batch,
        params.n_segments,
        params.n_fft,
...
fn mel_from_butterfly(
    ...
    let filters = crate::spectral::mel::mel_filterbank(n_fft, n_mels, sr);
    crate::spectral::mel::log_mel_from_spectrum_batch(&spec, &filters, batch, n_fft, n_mels)
}
