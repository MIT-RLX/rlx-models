...
use crate::model::pruned::{init_gates, pruned_forward_real_batch};
...
use crate::spectral::peak::{WelchPeakParams, welch_peaks_from_segment_spectrum};
use crate::spectral::welch::{
    WelchParams, average_welch_psd, welch_rustfft, welch_windowed_segments,
};
...
impl FastLearnedFftModel {
    ...
    fn gates_for_inference(&self) -> Vec<f32> {
        match self.hard_gate_threshold {
            Some(t) => crate::model::pruned::hard_gates(&self.gates, t),
    ...
    pub fn mean_gate(&self) -> f32 {
        crate::model::pruned::mean_gate(&self.gates)
    }

    pub fn active_gates(&self, threshold: f32) -> usize {
        crate::model::pruned::active_gate_count(&self.gates, threshold)
    }
}
