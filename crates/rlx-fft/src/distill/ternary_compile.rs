...
use crate::model::butterfly::split_complex_planes;
use crate::model::config::FftLearnConfig;
use crate::model::pruned::{
    active_ternary_butterfly_count, append_ternary_butterfly_from_real_signal,
};
...
fn bake_band(
    c: &crate::distill::ternary_arch::SpectrumCorrection,
    freq_mask: &[f32],
) -> Result<(Vec<f32>, Vec<f32>)> {
    use crate::distill::ternary_arch::SpectrumCorrection;
    match c {
...
pub fn compile_distilled_ternary_mel(
    ...
    let store = crate::model::weights::WeightStore::from_twiddles(&model.twiddles, cfg.n_fft);
    ...
    mel_compiled.set_param("hann", &crate::spectral::mel::hann_window(cfg.n_fft));
    ...
    Ok(CompiledDistilledTernaryMel {
        ...
        n_mels: model.n_mels,
        batch: cfg.batch,
        compute_fraction: crate::distill::ternary_gates::compute_fraction(&mel_gates),
...
mod tests {
    ...
    fn compiled_ternary_all_forward_spectrum_matches_eager() {
        ...
        let w = crate::spectral::mel::hann_window(64);
    ...
    fn compiled_ternary_pruned_spectrum_matches_eager_n128() {
        ...
        let w = crate::spectral::mel::hann_window(128);
        let mut windowed = signal.clone();
        for b in 0..8 {
            for i in 0..128 {
