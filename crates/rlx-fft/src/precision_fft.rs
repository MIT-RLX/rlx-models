...
pub fn dd_fft(a: &mut [Cdd], sign: f64) {
    ...
    for i in 0..n {
        let j = crate::butterfly::bit_reverse(i, bits);
        if i < j {
            a.swap(i, j);
        }
    }
...
// round-to-base error-free transforms.
...
mod tests {
    ...
    fn twosum_survives_the_compiler() {
        ...
        let mut exec = crate::compile::try_compile_graph(Device::Cpu, g).unwrap();
    ...
    fn compiled_f64_compensated_fft_runs() {
        ...
        let mut exec = crate::compile::try_compile_graph(Device::Cpu, graph).unwrap();
    ...
    fn fma_and_compensated_fft_on_metal() {
        ...
        let mut exec = crate::compile::try_compile_graph(Device::Metal, g).unwrap();
        ...
        let (n_fft, batch) = (8usize, 4usize);
        let (graph, params) = super::build_compensated_forward_graph(n_fft, batch).unwrap();
        let mut exec = crate::compile::try_compile_graph(Device::Metal, graph).unwrap();
        for (name, data) in &params {
            exec.set_param(name, data);
        }
        let mut rng = StdRng::seed_from_u64(3);
    ...
    fn compensated_fft_graph_runs_on_wgpu() {
        ...
        let (n_fft, batch) = (8usize, 4usize);
        let (graph, params) = super::build_compensated_forward_graph(n_fft, batch).unwrap();
        let mut exec = crate::compile::try_compile_graph(Device::Gpu, graph).unwrap();
        for (name, data) in &params {
            exec.set_param(name, data);
        }
        let mut rng = StdRng::seed_from_u64(3);
    ...
    fn fma_op_is_single_rounded_on_wgpu() {
        ...
        let e = g.add_node(Op::Fma, vec![a, b, neg_p], s);
        g.set_outputs(vec![p, e]);
        let mut exec = crate::compile::try_compile_graph(Device::Gpu, g).unwrap();
        let v = 1.0 + 2.0f32.powi(-13);
        let out = exec.run(&[("a", &[v; 4]), ("b", &[v; 4])]);
        assert!(
            out[1][0].abs() > 1e-10 && out[1][0].abs() < 1e-4,
