...
use crate::bench::e2e::E2ePipeline;
use crate::bench::sweep::available_devices;
use crate::distill::model::DistilledFftModel;
use crate::distill::ternary_arch::TernaryArchConfig;
use crate::distill::ternary_compile::{CompiledDistilledTernaryMel, compile_distilled_ternary_mel};
...
use crate::learned::model::{pipeline_max_err, ref_spectrum_batch, ref_welch};
...
use crate::spectral::mel::{log_mel_from_windowed_batch, ref_log_mel_batch};
use crate::spectral::peak::{WelchPeakParams, peaks_from_psd_batch, welch_peaks_rustfft};
use crate::spectral::welch::{WelchParams, welch_rustfft};
use crate::train::distill::{DistillTrainConfig, distill_from_teacher};
use crate::train::distill_ternary::{DistillTernaryTrainConfig, distill_ternary_from_distilled};
...
pub fn run_ternary_ablation(opts: &TernaryAblationOpts) -> Result<TernaryAblationReport> {
    ...
    for &n_fft in &opts.n_ffts {
        for &batch in &opts.batches {
            ...
            let windowed = crate::learned::compile::window_batch(&signal, batch, n_fft);
     ...
     -> Result<Vec<f32>> {
        match variant {
                ...
                match pipeline {
                    E2ePipeline::Mel => {
                        ...
                        crate::spectral::mel::log_mel_from_spectrum_batch(
                            &spec,
                            &crate::spectral::mel::mel_filterbank(n_fft, n_mels, sr),
                            batch,
                            n_fft,
                            n_mels,
                        )
            ...
            TernaryArchVariantId::RefButterflyCompiled => match pipeline {
                ...
                E2ePipeline::Welch => {
                    crate::spectral::welch::welch_butterfly(welch_signal, &tw, batch, *welch_params)
                }
                E2ePipeline::WelchPeaks | E2ePipeline::WelchPeaksUltra => {
