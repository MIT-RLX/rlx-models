...
use crate::train::e2e::E2eTrainConfig;
use anyhow::Result;
use rand::prelude::*;
...
fn train_fast_learned_model_with_curve(
    cfg: &E2eTrainConfig,
) -> Result<(crate::learned::model::FastLearnedFftModel, TracedE2eReport)> {
    use crate::learned::model::FastLearnedFftModel;
    use crate::model::pruned::pruned_forward_real_batch;
    use crate::model::twiddle_stability::project_twiddles_unit_circle;
    use crate::spectral::mel::ref_log_mel_batch;
    ...
    for step in 0..cfg.steps {
        ...
        let lr = crate::model::twiddle_stability::lr_for_n_fft(cfg.lr, cfg.n_fft);
        if step % 2 == 0 {
...
fn sample_loss_landscape(
    model: &crate::learned::model::FastLearnedFftModel,
    cfg: &E2eTrainConfig,
    seed: u64,
    ...
    for (i, &fx) in x.iter().enumerate() {
        for (j, &fy) in y.iter().enumerate() {
            ...
            let pred_mel = crate::spectral::mel::log_mel_from_spectrum_batch(
                &spec,
                model.mel_filters(),
                cfg.batch,
                cfg.n_fft,
...
fn trace_domain_twiddle(
    ...
    use crate::model::domain::domain_batch;

    let cfg = FftLearnConfig::new(n_fft, batch)?;
    let mut tw = crate::model::twiddle::exact_twiddles(&cfg);
    let mut rng = rand::rngs::StdRng::seed_from_u64(seed);
    let mut curve = Vec::new();
    ...
    for step in 0..steps {
        ...
        crate::model::butterfly::butterfly_train_step(&signal, &mut tw, batch, n_fft, 5e-4)?;
    ...
    Ok(ModelTrainingTrace {
        ...
        params,
        loss_curve: curve,
        heatmap: gate_heatmap_from_vec(&vec![1.0; crate::model::pruned::gate_count(n_fft)], n_fft),
        landscape: None,
...
fn trace_unitary(
    ...
    let cfg = FftLearnConfig::new(n_fft, batch)?;
    let (weights, final_err) = crate::model::unitary::train_unitary_quick(&cfg, steps, 1e-3, seed)?;
