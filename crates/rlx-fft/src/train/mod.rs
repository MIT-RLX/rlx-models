...
//!
//! Submodules cover the specialized regimes: phased / multi-stage schedules,
//! distillation, end-to-end pipelines, RLX-graph training, and the
//! second-order twiddle optimizer.

pub mod distill;
pub mod distill_ternary;
pub mod e2e;
pub mod fused;
pub mod graph;
pub mod multi;
...
use crate::model::config::{EncDecTrainConfig, FftLearnConfig, TrainConfig, TransformDir};
use crate::model::reference::{
    fft_real_batch, ifft_complex_batch, max_abs_error, mse, roundtrip_scale,
};
...
pub fn train_butterfly_dir(cfg: &TrainConfig, dir: TransformDir) -> Result<TrainResult> {
    if eager_train_from_env() {
        ...
        crate::train::rlx::train_butterfly_rlx(cfg, dir)
    }
}
...
pub fn train_encdec(cfg: &EncDecTrainConfig) -> Result<EncDecTrainResult> {
    if eager_train_from_env() {
        ...
        crate::train::rlx::train_encdec_rlx(cfg)
    }
}
...
pub fn load_weights(path: &Path) -> Result<WeightStore> {
    crate::model::weights::load_safetensors(path)
}
