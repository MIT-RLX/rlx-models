...
use crate::model::butterfly::{build_butterfly_forward_graph, butterfly_forward_real_batch};
use crate::model::config::FftLearnConfig;
use crate::model::domain::train_domain_twiddles;
use crate::model::fused::{build_fused_spectral_graph, fused_spectral_eager, unit_mask};
use crate::model::q8::Q8Twiddles;
use crate::model::reference::{fft_real_batch, max_abs_error};
use crate::model::stockham::{build_stockham_forward_graph, stockham_forward_real_batch};
use crate::model::twiddle::exact_twiddles;
use crate::model::unitary::{UnitaryWeights, train_unitary_quick};
    ...
    pub fn prepare(
        ...
        match variant {
            ...
            FftVariantId::RlxOpFft if self.compiled_rlx.is_none() => {
                self.compiled_rlx = Some(crate::exec::rlx_fft::compile_rlx_fft(
                    cfg,
                    crate::model::config::TransformDir::Forward,
                    device,
                )?);
            }
            FftVariantId::RlxOpIfft if self.compiled_rlx_inv.is_none() => {
                self.compiled_rlx_inv = Some(crate::exec::rlx_fft::compile_rlx_fft(
                    cfg,
                    crate::model::config::TransformDir::Inverse,
                    device,
                )?);
            }
    ...
    pub fn forward(
        ...
        match variant {
            ...
            FftVariantId::RlxOpFft => {
                ...
                Ok(crate::exec::rlx_fft::rlx_fft_forward(
                    exec, signal, batch, n,
                ))
            }
    ...
    pub fn inverse(&mut self, variant: FftVariantId, cfg: &FftLearnConfig) -> Result<Vec<f32>> {
        ...
        match variant {
            FftVariantId::Rustfft => {
                crate::model::reference::ifft_complex_batch(&self.inverse_spectrum, batch, n)
            }
            FftVariantId::RlxOpIfft => {
                let exec = self.compiled_rlx_inv.as_mut().expect("rlx inv compiled");
                Ok(crate::exec::rlx_fft::rlx_fft_inverse_block(
                    exec,
                    &self.spectrum_block,
                    batch,
                    n,
            ...
            FftVariantId::ButterflyEager => {
                crate::model::butterfly::butterfly_inverse_complex_batch(
...
fn compile_stockham(
    ...
    let store = WeightStore::from_twiddles(twiddles, cfg.n_fft);
    let mut compiled = crate::exec::compile::try_compile_graph(device, graph)?;
    store.apply_butterfly(&mut compiled, cfg.batch, cfg.n_fft);
    Ok(compiled)
}
...
pub fn variant_inverse_error(
    ...
    let target =
        crate::model::reference::ifft_complex_batch(&state.inverse_spectrum, cfg.batch, cfg.n_fft)?;
    Ok(max_abs_error(&pred, &target))
}
