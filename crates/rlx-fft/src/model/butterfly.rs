...
use crate::model::twiddle::{TwiddleSet, twiddle_index, twiddle_packed_name};
use crate::model::twiddle_stability::{DEFAULT_GRAD_CLIP, apply_twiddle_update, lr_for_n_fft};
...
pub fn butterfly_train_step_encdec(
    ...
    for b in 0..batch {
        ...
        let ref_spec = crate::model::reference::fft_real_batch(x, 1, n_fft)?;
    ...
    let effective_lr = lr_for_n_fft(lr as f64, n_fft);
    apply_twiddle_update(
        encoder_tw,
        &enc_tw_grad,
        effective_lr,
        DEFAULT_GRAD_CLIP,
...
pub fn butterfly_train_step(
    ...
    butterfly_train_complex_dir(
        ...
        |b| crate::model::reference::fft_real_batch(&signal[b * n_fft..(b + 1) * n_fft], 1, n_fft),
        twiddles,
        batch,
        n_fft,
        lr,
...
pub fn butterfly_train_step_inverse(
    ...
    butterfly_train_complex_dir(
        ...
        |b| {
            crate::model::reference::ifft_complex_batch(
...
fn butterfly_train_complex_dir(
    ...
    let effective_lr = lr_for_n_fft(lr as f64, n_fft);
    apply_twiddle_update(twiddles, &tw_grad, effective_lr, DEFAULT_GRAD_CLIP, true);
...
mod tests {
    ...
    use crate::model::twiddle_stability::max_twiddle_magnitude;
    use crate::train::random_batch;
    use rand::SeedableRng;

    #[test]
    fn encdec_500_steps_no_nan_at_legacy_lr() {
        let n = 256usize;
        let batch = 1;
        ...
        for step in 0..500 {
            ...
            let loss =
                butterfly_train_step_encdec(&signal, &mut enc, &mut dec, batch, n, 0.002, 0.25)
                    .unwrap();
            assert!(loss.reconstruction.is_finite(), "step {step} recon NaN");
            assert!(loss.spectrum.is_finite(), "step {step} spec NaN");
            assert!(enc.iter().all(|v| v.is_finite()), "step {step} enc NaN");
            assert!(dec.iter().all(|v| v.is_finite()), "step {step} dec NaN");
            assert!(max_twiddle_magnitude(&enc) <= 1.01);
            assert!(max_twiddle_magnitude(&dec) <= 1.01);
        }
    }
}
