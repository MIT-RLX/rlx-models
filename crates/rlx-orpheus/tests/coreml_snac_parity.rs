//! Native RLX CoreML (`Device::Ane`) SNAC decoder vs host eager parity.
...
#[test]
fn ane_snac_matches_eager() {
    let Some(path) = snac_path() else {
        eprintln!("skip ane_snac_matches_eager: set ORPHEUS_SNAC_PATH");
        return;
    };
    let Some(codes) = golden_frame_codes() else {
        eprintln!("skip ane_snac_matches_eager: set ORPHEUS_SNAC_REF_DIR with ref_codes.json");
        return;
    };
    ...
    let maxdiff = ref_pcm
        ...
        .map(|(a, b)| (a - b).abs())
        .fold(0.0f32, f32::max);
    eprintln!("ane-vs-eager SNAC maxdiff = {maxdiff:.3e}");
    assert!(
        maxdiff < 0.08,
        "native Ane SNAC diverged from eager (maxdiff={maxdiff:.3e})"
    );
