...
use rlx_voxtral_tts::backbone::rope::{build_inv_freq, build_rope_tables};
...
const ATTN_PROJS: [&str; 4] = ["wq", "wk", "wv", "wo"];
const FFN_PROJS: [&str; 3] = ["w1", "w2", "w3"];
...
fn ffn_lora_shapes(proj: &str, rank: usize, h: usize, ffn: usize) -> (Vec<usize>, Vec<usize>) {
    match proj {
        "w1" | "w3" => (vec![rank, h], vec![rank, ffn]),
        "w2" => (vec![rank, ffn], vec![rank, h]),
        _ => (vec![rank, h], vec![rank, h]),
    }
}

/// Expand KV heads for GQA so Metal SDPA indexes K/V with the same head count as Q.
fn repeat_kv(
    g: &mut Graph,
    x: NodeId,
    num_kv_heads: usize,
...
pub fn build_lora_train_graph(
    ...
    let ffn = text.intermediate_size.unwrap_or(h * 3);
    let layers = n_layers.min(text.num_hidden_layers);
    let dh = text.head_dim;
    let q_dim = text.num_attention_heads * dh;
    let kv_dim = text.num_key_value_heads * dh;
    let eps = text.rms_norm_eps as f32;

    let mut g = Graph::new("voxtral_lora");
    ...
    let inv = build_inv_freq(text.rope_theta, dh);
    let half = inv.len();
    let (rope_cos, rope_sin) = build_rope_tables(&inv, text.max_position_embeddings);
    let rope_cos_id = g.param(
        "rope.cos",
        Shape::new(&[text.max_position_embeddings, half], f),
    );
    ...
    for li in 0..layers {
        ...
        for proj in ATTN_PROJS {
            let (a_shape, b_shape): ([usize; 2], [usize; 2]) = match proj {
                "wq" => ([rank, h], [rank, q_dim]),
                "wk" | "wv" => ([rank, h], [rank, kv_dim]),
                "wo" => ([rank, q_dim], [rank, h]),
                _ => ([rank, h], [rank, h]),
            };
        ...
        let kv_group = text.num_attention_heads / text.num_key_value_heads;
        let k_rep = repeat_kv(&mut g, k3, text.num_key_value_heads, dh, kv_group);
        let v_rep = repeat_kv(&mut g, v3, text.num_key_value_heads, dh, kv_group);
        ...
        let ffn_norm = g.param(format!("{prefix}.ffn_norm.weight"), Shape::new(&[h], f));
        let w1 = g.param(
        ...
        let gate = proj_with_lora(&mut g, hn2, w1, ffn_lora[0].1, ffn_lora[0].2);
        let up = proj_with_lora(&mut g, hn2, w3, ffn_lora[2].1, ffn_lora[2].2);
        let gate_act = g.silu(gate);
        let prod = g.mul(gate_act, up);
        let ff = proj_with_lora(&mut g, prod, w2, ffn_lora[1].1, ffn_lora[1].2);
        x = g.add(x, ff);
    }
