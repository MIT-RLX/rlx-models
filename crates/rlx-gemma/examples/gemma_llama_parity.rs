...
fn main() -> Result<()> {
    ...
    let llama_greedy =
        rlx_gemma::llama_reference::greedy_generation_ids(&path, &ids, STEPS as u32, 512)?;
    ...
    println!(
        "hidden(CPU): cos={hc:.6} L2={hl2:.4} rlx[0..4]={:?} llama[0..4]={:?}",
        &rlx_h[..4.min(rlx_h.len())],
        &llama_h[..4.min(llama_h.len())]
    );
    ...
    if is_available(Device::Metal) {
        ...
        println!(
            "  rlx-CPU   : top1_match={} greedy_prefix={}",
            cpu_t5[0] == llama_t5[0],
            n(&cpu_g, &llama_greedy)
        );
        println!(
            "  rlx-Metal : top1_match={} greedy_prefix={}",
            m_t5[0] == llama_t5[0],
            n(&m_g, &llama_greedy)
        );
    }
